...
#: :mod:`twitchoglc.arena` already uses for a death with no killer.
NOBODY = ''
...
#: :mod:`twitchoglc.avatar` — so what you can hit is what can walk.  Declared
...
class Hit:
    ...
    **A hit on the level is a hit.**  It is where an impact effect is drawn, an
    ...
    #: :class:`~twitchoglc.collision.MapCollision`, and for geometry whose
...
def fire(world: Any, arena: Any, shooter: str, weapon: Any,
    ...
    ``surfaces`` is the :class:`~twitchoglc.collision.MapCollision` the level
