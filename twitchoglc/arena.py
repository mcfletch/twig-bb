...
class Fired:
    ...
    #: Who fired, and which weapon by its :attr:`~twitchoglc.weapons.Weapon.key`.
    shooter: str
    weapon: str
...
class PickedUp:
    ...
    #: :attr:`~twitchoglc.items.ItemKind.key`.
    target: str
    key: str
...
class Combatant:
    ...
    :class:`~twitchoglc.player.PlayerState` rather than being copied here: the
...
class Arena:
    ...
    Built by the start screen from a :class:`~twitchoglc.match.MatchSetup` and
    ...
    def add(self, id: str, position: Any = (0.0, 0.0, 0.0),
        ...
        made = Combatant(id=id, name=name or id, bot=bot, difficulty=difficulty,
                         player=PlayerState.carrying(self.weapons),
    ...
    def respawn(self, id: str, position: Any) -> Optional[Combatant]:
        ...
        **The same record comes back, restored** rather than replaced.  Every
        ...
        :meth:`~twitchoglc.player.PlayerState.restore`.
        """
