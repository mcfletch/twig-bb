...
class ItemKind(node.Node):
    ...
    #: :class:`~twitchoglc.weapons.Weapon`'s ``ammoType`` uses.
    ...
    #: The model, relative to :data:`twitchoglc.art.ASSETS`, and how to place
...
class Pickups:
    ...
    def advance(self, arena: Any, dt: float) -> List[Taken]:
        ...
        for item in self.items:
            if item.waiting is not None:
                ...
                if item.waiting <= 0.0:
                    item.waiting = None
        return self._collect(arena)

    def _collect(self, arena: Any) -> List[Taken]:
        ...
        for item in self.items:
            ...
            for id, feet in standing:
                ...
                if one is None or not item.kind.give_to(one.player):
