...
controller walks on and what :mod:`twitchoglc.combat` casts against.
...
:class:`~twitchoglc.worldgeometry.SurfaceIndex` built from the same batches in
...
#: map's: a map states its own in :mod:`twitchoglc.maploader` and the
