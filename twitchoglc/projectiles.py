...
Splash damage, knockback and rocket jumps are :mod:`twitchoglc.blast`, which
answers the :class:`~twitchoglc.arena.Detonated` events this emits.
"""
...
class Projectile(node.Node):
    ...
    #: (see :func:`twitchoglc.blast.burst`), so that a hit is one hit with one
    ...
    #: pushes there in metres per second.  Read by :mod:`twitchoglc.blast`.
