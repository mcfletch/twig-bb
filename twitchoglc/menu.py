...
def main_menu(on_play: Optional[Callable[[], None]] = None,
              ...
              subtitle: str = '') -> Panel:
    """The first screen: what a player can do from a standing start.

    Not dismissable by Escape and with no Cancel, because there is nothing
    behind it to go back to — closing it is what Quit is for.
    """
    buttons = [
        ('play', 'Play', on_play, True),
    ...
    for name, text, handler, primary in buttons:
...
def play_screen(setup: match.MatchSetup, levels: Sequence[match.Level],
    ...
    **Edits a draft.** The setup handed in is left alone until Start is
    ...
    :class:`~twitchoglc.match.MatchSetup` appears here with no edit to this
    function.
    """
