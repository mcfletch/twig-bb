...
Everything between the rules — which are :mod:`twitchoglc.arena`,
:mod:`twitchoglc.combat` and :mod:`twitchoglc.bots`, and know nothing about
...
#: ``walkSpeed`` of the capsule :mod:`twitchoglc.walkers` gives it, so a bot
...
def _spawns(loaded: Any) -> List[np.ndarray]:
    ...
    :func:`twitchoglc.avatar.feet_of`, which is the *same* one the camera's
...
def step_bots(world: Any, match: arenamod.Arena,
    ...
    ``surfaces`` is the map's :class:`~twitchoglc.collision.MapCollision`, so
    ...
    ``walking`` is the :class:`~twitchoglc.walkers.Walkers` that gives each of
