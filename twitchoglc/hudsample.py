...
``twitch-hud-demo`` -- the stand-in for the game twitch is on the way to being.
...
class HUDSampleContext(OverlayMixin, BaseContext):      # pragma: no cover - GL
    ...
    def _screenshot(self, event: Any = None) -> None:
        ...
        name = time.strftime('twitch-hud-%Y%m%d-%H%M%S.png')
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='the twitch HUD, over a room, with a weapon in hand')
    ...
    parser.add_argument('--weapon', metavar='KEY', default=None,
                        help='start holding this weapon (see the table in '
                             'twitchoglc/weapons.py); handy for a screenshot '
                             'of each')
...
def main() -> int:                              # pragma: no cover - needs a window
    ...
    definition = ContextDefinition(title='twitch HUD sample', size=(1280, 720),
                                   vsync=False)
