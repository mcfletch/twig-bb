...
Facts under test are SPEC-BSP46 §6.1/§7.3 and SPEC-Q3SHADER §1.6.
"""
...
def test_a_world_texture_is_looked_up_by_its_rooted_name(content):
    """SPEC-BSP46 §6.1: a v46 name is already rooted at the archive."""
    library = MaterialLibrary([str(content)])
    assert library.texture_size('textures/xenos/comptile') == (16, 8)
...
def test_the_extension_search_order_prefers_tga_over_jpg(content):
    ...
    library = MaterialLibrary([str(content)])
    assert library.texture_size('textures/xenos/comptile') == (16, 8)    # the .tga wins


def test_a_jpg_is_found_when_no_tga_exists(content):
    write_image(content / 'textures' / 'bg' / 'tile.jpg', size=(4, 4))
    library = MaterialLibrary([str(content)])
    assert library.texture_size('textures/bg/tile') == (4, 4)


def test_a_missing_texture_reports_the_documented_fallback_size(content):
    """SPEC-BSP46 §6.2 divides by the real size, so a missing image is guessed."""
    library = MaterialLibrary([str(content)])
...
def test_the_base_colour_map_is_sampled_as_srgb(content):
    ...
    library = MaterialLibrary([str(content)])
    material = library.material_for(SurfaceStyle(name='textures/xenos/comptile'))
...
def test_a_translucent_style_produces_a_blended_material(content):
    """SPEC-Q3SHADER §2.2: a translucent surface is drawn blended."""
    library = MaterialLibrary([str(content)])
    material = library.material_for(SurfaceStyle(name='textures/xenos/comptile', opacity=1 / 3))
...
def test_a_masked_style_produces_a_cut_out_material(content):
    ...
    library = MaterialLibrary([str(content)])
    material = library.material_for(SurfaceStyle(name='textures/xenos/comptile', masked=True))
...
def test_the_lightmap_strength_is_configurable(content):
    ...
    library = MaterialLibrary([str(content)], lightmap_strength=3.5)
    material = library.material_for(SurfaceStyle(name='x'),
                                    lightmap=np.zeros((4, 4, 3), np.uint8))
...
def test_an_unlit_style_gets_no_lightmap_even_with_a_page(content):
    """SPEC-Q3SHADER §2.2: a sky or warped surface carries no lightmap."""
    library = MaterialLibrary([str(content)])
...
def test_an_image_is_loaded_once_and_shared(content):
    ...
    library = MaterialLibrary([str(content)])
    first = library.material_for(SurfaceStyle(name='textures/xenos/comptile'))
    second = library.material_for(SurfaceStyle(name='textures/xenos/comptile'))
    assert first.textures['baseColor'] is second.textures['baseColor']


def test_a_material_is_built_once_per_style(content):
    library = MaterialLibrary([str(content)])
    style = SurfaceStyle(name='textures/xenos/comptile')
...
def test_several_content_roots_are_searched_in_order(tmp_path):
    ...
    library = MaterialLibrary([str(first), str(second)])
    assert library.texture_size('textures/x/y') == (8, 8)
...
def test_a_texture_is_found_when_only_its_case_differs(content):
    ...
    library = MaterialLibrary([str(content)])
    assert library.resolve('textures/natestah/nateweb') is not None
    assert library.texture_size('textures/natestah/nateweb') == (2, 2)
...
def test_an_exactly_matching_name_still_wins_over_a_differently_cased_one(content):
    ...
    library = MaterialLibrary([str(content)])
    assert library.texture_size('textures/dup/wall') == (4, 4)
...
def test_a_name_whose_directory_case_differs_is_still_found(content):
    ...
    library = MaterialLibrary([str(content)])
    assert library.resolve('textures/mixedcase/floor') is not None
