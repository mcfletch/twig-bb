...
class Rules:
    ...
    def advance(self, world: Any, dt: float, weapon: Any, seed: int = 0,
        ...
        if self.pickups is not None:
            # The arena's table, because a weapon walked over goes into the
            # hand when it beats what is held, and `slot` is the order that
            # decides -- see `PlayerState.prefer`.
            self.pickups.advance(self.arena, step, self.arena.weapons)
