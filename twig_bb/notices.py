...
def full_text(current: Optional[MapNotice] = None) -> str:
    """Everything, in the order a reader wants it.

    ``current`` is the map being played, and it comes **first**: a player who
    opens this while standing in a level is asking about that level, and the
    attribution its licence requires is the one they can act on.  With nothing
    ...
    parts.extend([code_notices(), content_notices(), provenance()])
    return '\n'.join(parts)
