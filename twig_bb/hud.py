...
(:mod:`twig_bb.debug`), and the two must not drift back together: the whole
...
:class:`~twig_bb.player.PlayerState` and writes what it finds onto the
...
#: camera's -- see :data:`twig_bb.deathcam.WASH` -- because it comes up with
...
class GameHUD(HUDLayer):
    ...
    def scoreboard(self, lines: Sequence[str]) -> None:
        ...
        as given: :func:`twig_bb.game.scoreboard_lines` has already made
    ...
    def dying(self, strength: float) -> None:
        ...
        dying.  See :class:`twig_bb.deathcam.DeathCamera`.
        """
