...
rate, the renderer's state and the camera.  What is here is the half only twig-bb
...
def install(context: Any) -> None:
    """Register twig-bb's sections on a context's developer overlay.
...
def map_provider(context: Any) -> Any:
    ...
    def rows() -> List[Tuple[str, Any]]:
        ...
        # Who made it and under what terms.  Beside the name rather than
        # buried under the counts, because a level is somebody's work and the
        # licence it arrived under is the reason this is on screen at all --
        # see :mod:`twig_bb.mapnotice`.  Each row is left out when the map
        # said nothing, since a heading over a blank is worse than no row.
        notice = getattr(context, 'notice', None)
        if notice is not None:
            for label, value in (('title', notice.title),
...
def player_provider(context: Any) -> Any:
    ...
    def rows() -> List[Tuple[str, Any]]:
        ...
        # from one.  See `twig_bb.frameclock`.
