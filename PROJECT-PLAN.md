...
| B44 | **Rockets you could stroll around, bots that opened every fight the same way, and a rifle firing "cells" — fixed 2026-08-18.** Five reported faults, all in the combat feel. **(1) Dying with an empty rifle trapped you at the scoreboard.** The trigger ends a death (B24), but that path ran through the weapon accounting, and an empty gun answered `OUT OF CARTRIDGES` and never reached the respawn — and a loaded one spent a round every frame you held fire waiting to come back. `viewer._runCommands` now reads a corpse's trigger as a respawn request and never as a shot. **(2) The rocket was a flat 26 m/s and a sidestep beat it at any distance.** It is a *motor* now: `Projectile` gained `acceleration` and `maxSpeed`, applied along the heading at the end of each tick (so "leaves at `speed`" stays literally true), and the rocket leaves at 16 m/s and thrusts to 60 — slow enough up close to be a splash weapon aimed at the floor, fast enough at range that a step cannot answer it. `time_to`/`distance_in` are the companion kinematics a bot's lead and its `reach` now use, so a bot aims and gates on where the rocket *will* be rather than on its launch speed. **(3) Bots always opened on a rocket and never ran dry**, because their shots ignored the loadout their body carries. A bot chooses from, and spends, exactly that `PlayerState` now (`Bot._chosen` gating on `can_fire`, `game._apply` spending it), so — like a player — it spawns with the pistol and has to *find* a launcher, a bot and a player in view of each other spawn on the same footing, and a bot down to its pistol plays differently from one with a rocket in its pack. **(4) Running a weapon dry left you clicking it**; the hand now falls to the highest weapon still loaded (`PlayerState.to_best_loaded`), announced on the HUD like a chosen switch. **(5) The sniper rifle took "cells"**, which is a plasma word; the pool and what the HUD prints are `cartridges` now, while the Quake-3 map classnames (`ammo_cells` and the rest) are left as the format spells them. Red/Green throughout; the README's projectile table, weapon and bot sections carry the new behaviour. |
